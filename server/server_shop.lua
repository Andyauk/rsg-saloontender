...
AddEventHandler('rsg-saloontendershop:server:GetShopItems', function(data)
    ...
    MySQL.query('SELECT * FROM saloontendershop_stock WHERE shopid = ?', {data.id}, function(data2)
        MySQL.query('SELECT * FROM saloontender_shop WHERE shopid = ?', {data.id}, function(data3)
            TriggerClientEvent('rsg-saloontendershop:client:ReturnStoreItems', src, data2, data3)
        end)
    end)
end)
...
RSGCore.Functions.CreateCallback('rsg-saloontendershop:server:Stock', function(source, cb, playerjob)
